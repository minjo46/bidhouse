...
resource "aws_security_group" "rds_mysql" {
  ...
  # ✅ CodeBuild VPC SG에서만 3306 허용 (프라이빗 접근)
  ingress {
    description     = "Allow MySQL from CodeBuild via VPC"
    from_port       = 3306
    to_port         = 3306
    protocol        = "tcp"
    security_groups = [data.aws_security_group.codebuild_sg.id]
  }
